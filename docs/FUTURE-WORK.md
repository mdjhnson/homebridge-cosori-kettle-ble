...
**Review (2026-09-25, `/code-review` and `/security-review`):** fixed on the branch: signals are accepted only from the current owner of `org.bluez` and never when addressed to us directly (node-ble's library checked the sender; dbus-native doesn't); a drop during `connect()` no longer leaves it hanging or reporting a dead link as connected; encoding errors (which quote the message body, so could include the key) are replaced by a generic message; `openBus` waits for the handshake; `@homebridge/dbus-native` is pinned to 0.7.9 because the timeout cleanup uses its internal `cookies` map (a test checks it on the real client). Left for later: `findDevice` polls the full `GetManagedObjects` tree every 500 ms while scanning (use `InterfacesAdded` if it's ever slow); a bus that stays open but stops answering is never replaced; `dbusAddress` values starting with `unix:` may list more addresses after `;` (same on `main`, and whoever edits `config.json` can install any plugin anyway).

**Pi test (2026-09-25):** `cosori-probe adapters` and `scan`, child-bridge restarts (connects in about 2 s), and Green Tea on and off from the Home app all worked; see STATUS "Deployed on the Pi". Not yet seen on the new transport: a reconnect after a natural drop (none happened from 22:51 to 23:43).
